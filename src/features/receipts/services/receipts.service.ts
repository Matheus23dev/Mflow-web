...
import type { Receipt, ReceiptKind, ReceiptStorageStatus } from "@/shared/types";

export const receiptsService = {
  status: () => api<ReceiptStorageStatus>("/receipts/storage/status"),
  list: (loanId: string) => api<Receipt[]>(`/receipts/loans/${loanId}`),
  upload: (loanId: string, file: File, kind: ReceiptKind, paymentId?: string) => {
  ...
  file: (id: string) => apiBlob(`/receipts/${id}/file`),
  remove: (id: string) => api<{ success: boolean }>(`/receipts/${id}`, { method: "DELETE" }),
};
