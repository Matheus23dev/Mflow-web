...
export function ReceiptList({
  ...
  async function remove(receipt: Receipt) {
    if (!window.confirm(`Apagar o comprovante “${receipt.originalName}”?`))
      return;
    setBusy(receipt.id);
    setError("");
    try {
    ...
    <div className="receipt-list mt-4 min-w-0 border-t border-slate-100 pt-4">
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <span className="text-[10px] text-slate-400">
          {receipts.length} arquivo{receipts.length === 1 ? "" : "s"}
        </span>
      </div>
      {receipts.length ? (
        <div className="grid min-w-0 grid-cols-1 gap-2 min-[641px]:grid-cols-2">
          {receipts.map((receipt) => {
            ...
            return (
              <div
                key={receipt.id}
                className="flex min-w-0 items-center gap-2.5 rounded-xl border border-slate-200 bg-white p-3"
              >
        ...
        <p className="m-0 rounded-xl border border-dashed border-slate-200 bg-white px-3 py-3 text-center text-[10.5px] text-slate-400">
          Nenhum comprovante salvo neste contrato.
        </p>
      )}
