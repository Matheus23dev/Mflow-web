...
export function LoanFormModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: (loan: Loan) => void }) {
  ...
  useEffect(() => {
    if (open) customersService.list().then(setCustomers).catch((caught) => setError(caught.message));
  }, [open]);
...
export function EditLoanModal({ loan, onClose, onSaved }: { loan: Loan | null; onClose: () => void; onSaved: (updatedLoan: Loan) => void }) {
  const [principalAmount, setPrincipalAmount] = useState(() => loan?.principalAmount || "");
  const [principalBalance, setPrincipalBalance] = useState(() => loan?.principalBalance || loan?.principalAmount || "");
  ...
  const [installmentCount, setInstallmentCount] = useState(() => loan?.installmentCount?.toString() || "10");
  const [installmentAmount, setInstallmentAmount] = useState(() => loan?.installmentAmount?.toString() || "");
  const [firstDueDate, setFirstDueDate] = useState(() => loan?.firstDueDate ? loan.firstDueDate.split("T")[0] : "");
  const [monthlyDueDay, setMonthlyDueDay] = useState(() => loan?.monthlyDueDay?.toString() || "10");
  const [rateMode, setRateMode] = useState<"percentage" | "fixed">(() => loan?.monthlyInterestAmount && !loan.monthlyInterestRate ? "fixed" : "percentage");
  const [monthlyInterestRate, setMonthlyInterestRate] = useState(() => loan?.monthlyInterestRate?.toString() || "");
  const [monthlyInterestAmount, setMonthlyInterestAmount] = useState(() => loan?.monthlyInterestAmount?.toString() || "");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
            ...
            return (
              <article className={`loan-card min-w-0 overflow-hidden rounded-xl border bg-white transition ${isOpen ? "expanded border-violet-200 shadow-[0_8px_24px_rgba(78,53,130,.08)]" : "border-[#ebe8ef]"}`} key={loan.id}>
                <button className="loan-main grid w-full min-w-0 grid-cols-[35px_minmax(0,1fr)_minmax(78px,auto)_17px] items-center gap-2.5 border-0 bg-transparent px-3 py-3 text-left text-[#3b3642] min-[641px]:grid-cols-[35px_minmax(140px,1fr)_minmax(90px,.7fr)_minmax(85px,.6fr)_18px] min-[641px]:gap-3 min-[641px]:px-4 min-[861px]:grid-cols-[36px_minmax(140px,1.2fr)_minmax(115px,.75fr)_minmax(100px,.7fr)_minmax(95px,.6fr)_18px]" onClick={() => setExpanded(isOpen ? null : loan.id)}>
                  ...
                  <div className="loan-amount flex min-w-0 flex-col items-end text-right min-[641px]:items-start min-[641px]:text-left"><strong className="max-w-full overflow-hidden text-[13px] text-violet-700 text-ellipsis whitespace-nowrap">{money(loan.principalAmount)}</strong><span className="mt-1 text-[10px] text-[#9b96a2]">valor emprestado</span></div>
