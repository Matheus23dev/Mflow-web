...
import { Avatar, Button, EmptyState, ErrorState, Field, Input, LoadingState, Modal, PageHeader, Select, StatusBadge, Textarea } from "@/shared/ui";
...
export function PaymentModal({ loanId, preset, onClose, onSaved }: { loanId: string | null; preset?: CollectionItem | null; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  useEffect(() => {
    if (!loanId) return;
    collectionsService.loan(loanId).then((result) => {
      setLoan(result);
      setError("");
    ...
    <Modal open={Boolean(loanId)} onClose={onClose} title="Registrar pagamento" description={loan ? `${loan.customer.name} · ${loan.type === "WEEKLY" ? "Contrato parcelado" : "Juros mensal"}` : "Atualizando dados do contrato"}>
      ...
      {loan && !loading ? (
        <form className={formGridClass} onSubmit={submit}>
          <div className={`${fieldSpanClass} payment-type-tabs grid min-w-0 max-w-full grid-cols-1 gap-[5px] rounded-[9px] bg-[#f1eff4] p-1 min-[421px]:flex`}>
            {paymentOptions.map((option) => <button key={option.value} type="button" className={`min-w-0 flex-1 [overflow-wrap:anywhere] rounded-[7px] border-0 px-2 py-2 text-[11px] font-bold ${paymentType === option.value ? "selected bg-white text-violet-700 shadow-[0_2px_7px_rgba(44,35,68,.08)]" : "bg-transparent text-[#817c89]"}`} onClick={() => selectType(option.value as PaymentType)}>{option.label}</button>)}
          </div>
  ...
  return (
    <div className="page-enter data-page collections-page min-w-0 max-w-full min-[861px]:flex min-[861px]:h-full min-[861px]:min-h-0 min-[861px]:flex-col min-[861px]:overflow-hidden">
      ...
      <div className="collection-summary mb-[17px] grid grid-cols-1 gap-[9px] min-[641px]:grid-cols-2 min-[641px]:gap-[15px]">
        <CollectionSummaryCard tone="purple" icon={<CalendarCheck2 size={20} />} label="No período" value={money(total)} detail={`${filtered.length} cobranças`} />
        <CollectionSummaryCard tone="red" icon={<Clock3 size={20} />} label="Em atraso" value={money(overdue)} detail={`${filtered.filter((item) => item.status === "OVERDUE").length} pendências`} />
      </div>
      <section className="panel table-panel min-w-0 overflow-hidden rounded-[14px] border border-[#e9e7ef] bg-white shadow-[0_3px_14px_rgba(42,35,65,0.025)] min-[861px]:flex min-[861px]:min-h-0 min-[861px]:flex-1 min-[861px]:flex-col">
        <div className="collection-toolbar flex flex-col items-stretch justify-between gap-4 border-b border-[#eceaf0] px-[18px] py-[14px] min-[641px]:flex-row min-[641px]:items-center"><div className="segmented grid grid-cols-3 gap-1 rounded-[9px] bg-[#f3f1f6] p-[3px] min-[641px]:grid-cols-5">{filters.map((item) => <button key={item.value} className={`min-h-9 min-w-0 rounded-[7px] border-0 px-1.5 py-[7px] text-[11.5px] font-semibold ${filter === item.value ? "active bg-white text-violet-700 shadow-[0_2px_7px_rgba(42,32,70,.08)]" : "bg-transparent text-[#888391]"}`} onClick={() => setFilter(item.value)}>{item.label}</button>)}</div><div className="search-box small flex h-[43px] w-full items-center gap-[9px] rounded-[9px] border border-[#e6e3ea] bg-[#faf9fc] px-[11px] text-[#9b97a3] focus-within:border-violet-300 focus-within:ring-4 focus-within:ring-violet-100 min-[641px]:w-[210px]"><Search className="shrink-0" size={17} /><input className="min-w-0 flex-1 border-0 bg-transparent text-[16px] text-[#373340] outline-none placeholder:text-[#aaa6b1] min-[641px]:text-[13px]" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar cliente" /></div></div>
        ...
        {filtered.length ? <div className="collection-table-head sticky top-0 z-[2] hidden grid-cols-[64px_minmax(160px,1fr)_minmax(150px,.9fr)_minmax(115px,.6fr)_176px] items-center gap-[11px] bg-white px-[22px] pb-2.5 pt-[13px] text-[10px] font-bold uppercase tracking-[.45px] text-[#9a95a2] min-[1051px]:grid min-[1121px]:grid-cols-[72px_minmax(190px,1.15fr)_minmax(180px,1fr)_minmax(135px,.65fr)_198px] min-[1121px]:gap-[14px] [&>span:nth-child(4)]:text-center [&>span:nth-child(5)]:text-center"><span>Vencimento</span><span>Cliente</span><span>Cobrança</span><span>Valor a receber</span><span>Ações</span></div> : null}
        <div className="collection-list grid gap-[9px] px-[7px] pb-2 min-[641px]:p-[9px] min-[861px]:min-h-0 min-[861px]:flex-1 min-[861px]:overflow-y-auto min-[861px]:overscroll-contain min-[1051px]:block min-[1051px]:px-3 min-[1051px]:pb-3 min-[1051px]:pt-0">
          {filtered.map((item) => (
            <article className={`collection-card grid grid-cols-[48px_minmax(0,1fr)] [grid-template-areas:'date_customer'_'charge_charge'_'total_total'_'actions_actions'] items-center gap-[11px] rounded-[11px] border p-[13px] min-[641px]:grid-cols-[52px_minmax(0,1fr)_minmax(140px,auto)] min-[641px]:[grid-template-areas:'date_customer_total'_'date_charge_total'_'._actions_actions'] min-[641px]:gap-x-3 min-[641px]:gap-y-2 min-[641px]:p-[14px] min-[1051px]:grid-cols-[64px_minmax(160px,1fr)_minmax(150px,.9fr)_minmax(115px,.6fr)_176px] min-[1051px]:[grid-template-areas:none] min-[1051px]:gap-[11px] min-[1051px]:rounded-none min-[1051px]:border-x-0 min-[1051px]:border-b-0 min-[1051px]:bg-white min-[1051px]:px-2.5 min-[1051px]:py-[14px] min-[1121px]:grid-cols-[72px_minmax(190px,1.15fr)_minmax(180px,1fr)_minmax(135px,.65fr)_198px] min-[1121px]:gap-[14px] ${item.status === "OVERDUE" ? "is-overdue border-rose-200 bg-[#fffafb] min-[1051px]:my-[5px] min-[1051px]:rounded-[11px] min-[1051px]:border" : "border-[#ece9f0] bg-white"}`} key={item.id}>
              <div className={`collection-date [grid-area:date] flex h-11 w-[42px] flex-col items-center justify-center rounded-[9px] border min-[641px]:self-start min-[1051px]:[grid-area:auto] min-[1051px]:self-center ${item.status === "OVERDUE" ? "border-rose-200 bg-rose-50 text-rose-600" : "border-[#e7e2f0] bg-[#f5f2ff] text-[#6342ca]"}`}><span className="text-sm font-extrabold leading-none">{new Date(item.dueDate).getUTCDate().toString().padStart(2, "0")}</span><small className="mt-1 text-[7px] font-bold uppercase">{new Intl.DateTimeFormat("pt-BR", { month: "short", timeZone: "UTC" }).format(new Date(item.dueDate)).replace(".", "")}</small></div>
              <div className="collection-customer [grid-area:customer] flex min-w-0 items-center gap-2.5 min-[1051px]:[grid-area:auto]"><Avatar name={item.customer.name} /><div className="flex min-w-0 flex-col"><strong className="overflow-hidden text-[13px] text-ellipsis whitespace-nowrap">{item.customer.name}</strong><span className="mt-1 flex items-center gap-1 text-[10.5px] text-[#98939e]"><Phone className="shrink-0" size={13} /> {item.customer.phone}</span></div></div>
              <div className="collection-charge [grid-area:charge] flex min-w-0 flex-col pt-0.5 min-[1051px]:[grid-area:auto] min-[1051px]:pt-0"><div className="flex flex-wrap items-center gap-[7px]"><strong className="text-[13px]">{item.label}</strong><StatusBadge status={item.status} /></div>{item.lateFee > 0 ? <span className="danger-text mt-[5px] text-[10.5px] leading-snug text-rose-500">{item.daysOverdue} dia{item.daysOverdue === 1 ? "" : "s"} · juros {money(item.lateFee)}</span> : item.paidAmount > 0 ? <span className="mt-[5px] text-[10.5px] leading-snug text-[#918c98]">Já recebido: {money(item.paidAmount)}</span> : null}</div>
              <div className="collection-total [grid-area:total] grid w-full grid-cols-[minmax(0,1fr)_auto] items-center rounded-[9px] bg-[#f7f5fa] px-3 py-2.5 text-left min-[641px]:flex min-[641px]:w-auto min-[641px]:flex-col min-[641px]:items-end min-[641px]:justify-center min-[641px]:bg-transparent min-[641px]:p-0 min-[641px]:text-right min-[1051px]:[grid-area:auto] min-[1051px]:items-center min-[1051px]:text-center"><span className="col-start-1 text-[10.5px] text-[#8e8996] min-[1051px]:hidden">A receber</span><strong className="col-start-2 row-span-2 row-start-1 m-0 text-[17px] text-[#3e3749] min-[641px]:mt-1 min-[641px]:text-base">{money(item.updatedAmount)}</strong>{item.lateFee > 0 ? <small className="col-start-1 text-[9px] text-[#a19ca6] min-[641px]:mt-[3px] min-[641px]:text-right">Parcela {money(item.outstanding)} + juros</small> : item.paidAmount > 0 ? <small className="col-start-1 text-[9px] text-[#a19ca6] min-[641px]:mt-[3px] min-[641px]:text-right">Original {money(item.originalAmount)}</small> : null}</div>
              <div className="collection-actions [grid-area:actions] grid grid-cols-[44px_minmax(0,1fr)] items-center justify-stretch gap-[7px] min-[1051px]:[grid-area:auto] min-[1051px]:flex min-[1051px]:justify-end"><a className="icon-button inline-grid size-10 place-items-center rounded-[10px] border border-[#e9e5ee] bg-white p-0 text-emerald-600 transition hover:bg-emerald-50" href={`https://wa.me/${whatsappPhone(item.customer.phone)}?text=${encodeURIComponent(reminderText(item))}`} target="_blank" rel="noreferrer" aria-label={`Enviar lembrete para ${item.customer.name}`}><MessageCircle size={18} /></a><Button className="w-full" onClick={() => onPayment(item)}><CircleDollarSign size={17} /> Receber</Button></div>
            </article>
          ))}
        </div>
      </section>
