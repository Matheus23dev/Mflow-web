    ...
    <div className="page-enter data-page cash-page min-w-0 max-w-full min-[861px]:flex min-[861px]:h-full min-[861px]:min-h-0 min-[861px]:flex-col min-[861px]:overflow-hidden">
      ...
      <section className="panel table-panel min-w-0 overflow-hidden rounded-[14px] border border-[#e9e7ef] bg-white shadow-[0_3px_14px_rgba(42,35,65,0.025)] min-[861px]:flex min-[861px]:min-h-0 min-[861px]:flex-1 min-[861px]:flex-col">
        <div className="list-toolbar cash-toolbar flex min-h-[68px] flex-col items-stretch justify-between gap-[14px] border-b border-[#eeecf1] px-[18px] py-[14px] min-[641px]:flex-row min-[641px]:flex-wrap min-[641px]:items-center">
          <div className="search-box flex h-[43px] w-full items-center gap-[9px] rounded-[9px] border border-[#e6e3ea] bg-[#faf9fc] px-[11px] text-[#9b97a3] focus-within:border-violet-300 focus-within:ring-4 focus-within:ring-violet-100 min-[641px]:w-[min(360px,100%)]"><Search className="shrink-0" size={19} /><input className="min-w-0 flex-1 border-0 bg-transparent text-[16px] text-[#373340] outline-none placeholder:text-[#aaa6b1] min-[641px]:text-[13px]" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por descrição ou cliente" /></div>
          <div className="date-range grid w-full grid-cols-1 items-center gap-[7px] text-[#97939f] min-[421px]:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] min-[641px]:flex min-[641px]:w-auto [&>svg]:hidden min-[641px]:[&>svg]:block [&>span]:hidden min-[421px]:[&>span]:block [&_[data-ui=input]]:w-full min-[641px]:[&_[data-ui=input]]:w-[132px]"><CalendarRange className="shrink-0" size={18} /><Input type="date" value={from} onChange={(event) => setFrom(event.target.value)} aria-label="Data inicial" /><span className="text-[8px]">até</span><Input type="date" value={to} min={from} onChange={(event) => setTo(event.target.value)} aria-label="Data final" /></div>
        </div>
        ...
        <div className="cash-list px-1.5 pb-[7px] min-[641px]:px-3 min-[641px]:pb-3 min-[861px]:min-h-0 min-[861px]:flex-1 min-[861px]:overflow-y-auto min-[861px]:overscroll-contain">
          {transactions.map((transaction) => (
            <article className="cash-row grid grid-cols-[35px_minmax(0,1fr)_auto] items-center gap-2 border-t border-[#efedf2] px-1.5 py-3 min-[641px]:grid-cols-[35px_minmax(180px,1.3fr)_minmax(85px,.55fr)_65px_minmax(90px,.6fr)] min-[641px]:gap-3 min-[641px]:px-2.5 min-[641px]:py-[13px]" key={transaction.id}>
              {transaction.loan?.customer ? <Avatar name={transaction.loan.customer.name} /> : <span className="avatar inline-grid size-9 shrink-0 place-items-center rounded-xl border border-violet-100 bg-gradient-to-br from-violet-50 to-violet-100 text-violet-700"><Landmark size={19} /></span>}
              <div className="flex min-w-0 flex-col">
                <strong className="overflow-hidden text-[13px] text-ellipsis whitespace-nowrap">{transaction.description}</strong>
                <span className="cash-source mt-[3px] hidden text-[10.5px] text-[#9995a1] min-[641px]:block">{transaction.loan?.customer.name || "Movimentação manual"}</span>
                <span className="cash-mobile-date mt-[3px] block text-[10.5px] text-[#9995a1] min-[641px]:hidden">{date(transaction.createdAt)}</span>
              </div>
              <time className="mt-[3px] hidden text-[10.5px] not-italic text-[#9995a1] min-[641px]:block">{date(transaction.createdAt)}</time>
              <span className={`transaction-type hidden w-max rounded-full px-2 py-1 text-[10px] font-bold min-[641px]:block ${transaction.type === "INCOME" ? "income bg-emerald-50 text-emerald-700" : "expense bg-rose-50 text-rose-700"}`}>{transaction.type === "INCOME" ? "Entrada" : "Saída"}</span>
              <strong className={`min-w-0 justify-self-end whitespace-nowrap text-[clamp(11px,3.6vw,13px)] ${transaction.type === "INCOME" ? "income-text text-emerald-600" : "expense-text text-rose-500"}`}>{transaction.type === "INCOME" ? "+" : "−"} {money(transaction.amount)}</strong>
            </article>
          ))}
        </div>
      </section>
      ...
      <Modal open={modalOpen} onClose={() => !saving && setModalOpen(false)} title="Movimentar o caixa" description="Registre dinheiro colocado ou retirado para outras necessidades." size="sm">
        <form className={`${formGridClass} cash-movement-form`} onSubmit={submit}>
          ...
          <div className={fieldSpanClass}><Field label="Descrição" hint="Ex.: despesa pessoal, aporte ou material" >
          ...
          {formError ? <p className={`${fieldSpanClass} form-error m-0 rounded-lg border border-rose-200 bg-rose-50 px-[11px] py-[9px] text-[11px] leading-relaxed text-rose-700`}>{formError}</p> : null}
