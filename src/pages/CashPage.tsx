...
export function CashPage({ refreshKey, onSaved }: { refreshKey: number; onSaved: (message: string) => void }) {
  ...
  const load = useCallback(() => {
    api<CashData>(`/cash${queryString({ from, to })}`)
      .then((result) => {
        setData(result);
        setPageError("");
      })
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await api("/cash", {
        method: "POST",
        body: JSON.stringify({ type, amount: Number(amount), description, transactionDate }),
      });
      setModalOpen(false);
      setAmount("");
      setDescription("");
  ...
  return (
    <div className="page-enter">
      ...
      <div className="cash-metrics">
        <article><span className="cash-icon purple"><Wallet size={22} /></span><div><p>Saldo do período</p><strong>{money(data?.summary.balance)}</strong></div></article>
        <article><span className="cash-icon green"><ArrowDownLeft size={22} /></span><div><p>Entradas</p><strong>{money(data?.summary.income)}</strong></div></article>
        <article><span className="cash-icon red"><ArrowUpRight size={22} /></span><div><p>Saídas</p><strong>{money(data?.summary.expense)}</strong></div></article>
      </div>
